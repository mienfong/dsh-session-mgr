 ...
 *   POST /dsh-session-mgr/import     { sourcePath, targetPath }   -> { ok, sessionId, importPath, cwd, attachments, logs, logEncoding, workspaceId?, workspaceTitle? }
...
async function importSession(ctx, sourcePath, targetPath) {
  ...
  try {
    ...
    // Install one log artifact, re-encoding it to the local container and
    // remapping the header `cwd` (both steps are container-aware).
