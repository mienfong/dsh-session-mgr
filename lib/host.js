 ...
 *   POST /dsh-session-mgr/import     { sourcePath, targetPath }   -> { ok, sessionId, importPath, cwd, attachments, logs, logEncoding, unknownEvents, workspaceId?, workspaceTitle? }
...
/**
 * Extract archive members into `destDir`, tolerating the two common layouts:
 *   - flat (the package files at the archive root), and
 *   - wrapped (Windows "Compressed Folder" puts everything under one
 *     top-level folder, e.g. `<sessionId>/manifest.json`).
 * Directory entries (names ending in `/`) are skipped; a common top-level
 * folder prefix is stripped so the package files land at the root.
 */
export async function extractMembersToDir(members, destDir) {
  const files = (members || []).filter((m) => m && typeof m.name === "string" && m.name.length > 0 && !m.name.endsWith("/"));
  let strip = "";
  if (files.length > 0) {
    const first = files[0].name.split("/")[0];
    const allSame = first.length > 0 && files.every((f) => f.name.split("/")[0] === first);
    // Only treat the shared folder as a wrapper when removing it exposes the
    // package root (a manifest.json or a session log). This keeps a legitimate
    // top-level folder such as `attachments/` from being stripped.
    const exposesRoot = allSame && files.some((f) => {
      const rel = f.name.startsWith(first + "/") ? f.name.slice(first.length + 1) : f.name;
      return rel === "manifest.json" || /^session\.jsonl(\.zstd)?$/.test(rel);
    });
    if (exposesRoot) strip = first;
  }
  for (const f of files) {
    let rel = f.name;
    if (strip && rel.startsWith(strip + "/")) rel = rel.slice(strip.length + 1);
    const dest = join(destDir, ...rel.split("/").filter(Boolean));
    await mkdir(dirname(dest), { recursive: true });
    await writeFile(dest, f.data);
  }
}

/** Read the session header (`{ id, cwd, createdAt, ... }`) from a session log file. */
export async function readLogHeader(logPath) {
  const compressed = logPath.endsWith(".jsonl.zstd");
  const raw = await readFile(logPath);
  let plaintext;
  if (compressed) {
    const { frames } = scanZstdFrames(raw);
    if (frames.length === 0) throw new Error("empty or header-less Zstandard session log");
    plaintext = zstdDecompressSync(raw.subarray(frames[0].start, frames[0].end)).toString("utf8");
  } else {
    plaintext = raw.toString("utf8");
  }
  const nl = plaintext.indexOf("\n");
  const line = nl === -1 ? plaintext : plaintext.slice(0, nl);
  return JSON.parse(line);
}

// ---------------------------------------------------------------------------
// Attachments — image/file attachments live OUTSIDE the session directory, in
// <DSH_HOME>/attachments/v1/<store>/<sha[0:2]>/<sha256>. A session log refers
// to them by `"attachmentId":"sha256:<64hex>"`, so a portable backup must
// carry the referenced objects too or the restored session cannot re-send any
// message that contains an image/file (ATTACHMENT_NOT_FOUND at request time).
// ---------------------------------------------------------------------------

/** Attachment id shape used by the harness: `sha256:<64 lowercase hex>`. */
const ATTACHMENT_ID_RE = /"attachmentId"\s*:\s*"(sha256:[0-9a-f]{64})"/g;

/** Root of the content-addressed attachment store: <DSH_HOME>/attachments/v1. */
export function attachmentStoreRoot() {
  return join(dshHomeDir(), "attachments", "v1");
}

/** Collect every distinct `sha256:<hex>` attachment id referenced by a session log. */
export function collectAttachmentIds(raw) {
  const ids = new Set();
  const scan = (text) => {
    ATTACHMENT_ID_RE.lastIndex = 0;
    let m;
    while ((m = ATTACHMENT_ID_RE.exec(text)) !== null) ids.add(m[1]);
  };
  if (raw.length >= 4 && raw.readUInt32LE(0) === ZSTD_MAGIC) {
    const { frames } = scanZstdFrames(raw);
    for (const f of frames) {
      try { scan(zstdDecompressSync(raw.subarray(f.start, f.end)).toString("utf8")); } catch { /* skip a torn frame */ }
    }
  } else {
...
/** Candidate store-relative paths for one attachment digest (image objects first). */
export function attachmentRelativePaths(hex) {
  const p = hex.slice(0, 2);
  return [`objects/${p}/${hex}`, `file-objects/${p}/${hex}`];
}

/** Recursively copy packaged attachments into the live store; content-addressed, so existing files are kept. */
export async function restoreAttachments(fromDir, toRoot) {
  let restored = 0;
  async function walk(abs, rel) {
    for (const entry of await readdir(abs, { withFileTypes: true })) {
      const childAbs = join(abs, entry.name);
      ...
      if (entry.isDirectory()) {
        ...
        const target = join(toRoot, ...childRel.split("/").filter(Boolean));
        await mkdir(dirname(target), { recursive: true });
        if (!(await exists(target))) {
          await cp(childAbs, target);
          restored += 1;
        }
      }
    }
...
// ---------------------------------------------------------------------------
// Session log physical encoding — the JSONL backend is configured for exactly
// one of `zstd` (default) and `none`, and rejects a session directory whose
// generation log uses the other one. `checkRootEncoding()` walks EVERY project
// directory, so one foreign-encoded artifact makes the harness reader throw for
// the whole root (the session list goes blank/500). A package therefore must be
// re-encoded to the LOCAL container on import, never copied verbatim.
// ---------------------------------------------------------------------------

/** Canonical session log names: `session.jsonl[.zstd]` and `session.vN.jsonl[.zstd]`. */
const GENERATION_LOG_RE = /^(session(?:\.v[1-9][0-9]*)?)\.jsonl(\.zstd)?$/;

/** Physical encoding of a session log filename (`zstd`/`none`), or undefined if it is not one. */
export function logEncodingOf(filename) {
  const match = GENERATION_LOG_RE.exec(filename);
  if (match === null) return undefined;
  return match[2] === ".zstd" ? "zstd" : "none";
}

/** The same generation log name expressed in `to`'s physical encoding. */
export function reencodeLogName(filename, to) {
  const match = GENERATION_LOG_RE.exec(filename);
  if (match === null) return filename;
  return `${match[1]}.jsonl${to === "zstd" ? ".zstd" : ""}`;
}

/**
 * Canonical log filename for a declared Session format version and physical
 * encoding. The backend cross-checks the two ("session generation filename
 * identifies v0, but its header identifies v3"), so a package whose filename
 * and header disagree has to be renamed on import.
 */
export function sessionLogName(version, encoding) {
  const generation = Number.isSafeInteger(version) && version > 0 ? `.v${version}` : "";
  return `session${generation}.jsonl${encoding === "zstd" ? ".zstd" : ""}`;
}

/**
 * Re-encode one session log between the plaintext and concatenated-zstd-frame
 * containers. Plaintext is preserved byte for byte (the header line, every
 * event line and its sequence numbering are untouched); only the container
 * changes. The zstd layout mirrors the backend exactly: the header line alone
 * in the first frame, the remaining lines in the following frames.
 */
export function convertLogEncoding(raw, from, to) {
  if (from === to || raw.length === 0) return raw;
  if (from === "zstd") {
    const { frames } = scanZstdFrames(raw);
    return Buffer.concat(frames.map((frame) => zstdDecompressSync(raw.subarray(frame.start, frame.end))));
  }
  const text = raw.toString("utf8");
  const cut = text.indexOf("\n");
  const head = cut === -1 ? text : text.slice(0, cut + 1);
  const rest = cut === -1 ? "" : text.slice(cut + 1);
  const frames = [zstdCompressSync(Buffer.from(head, "utf8"), CHECKSUM_OPTIONS)];
  if (rest.length > 0) frames.push(zstdCompressSync(Buffer.from(rest, "utf8"), CHECKSUM_OPTIONS));
  return Buffer.concat(frames);
}

// ---------------------------------------------------------------------------
// Event-vocabulary compatibility — a package exported by a NEWER harness can
// carry event types this machine does not know. The JSONL backend refuses such
// a session fail-closed unless the event envelope carries the compatibility
// marker `ignorable: true` (the contract documented on `SessionEvent.ignorable`
// in @deepseek-ai/dsh-session): a marked unknown event is retained verbatim but
// skipped, so the conversation replays unchanged. Writers of a new event type
// are expected to set that marker themselves; when they do not, importing the
// package verbatim installs a session directory the harness then refuses to
// open ("contains event type ... unknown to this harness and not marked
// ignorable") even though the plugin reported success. Import therefore marks
// those events on the user's behalf and reports what it had to skip.
// ---------------------------------------------------------------------------

/**
 * Add the `ignorable` compatibility marker to every event whose type this
 * harness does not know, so a log written by a newer harness stays readable
 * here. Only frames that contain such an event are re-compressed; the header
 * frame and every untouched frame keep their original bytes and checksum, and
 * an event that is known, already marked or unparsable is left exactly as it
 * is. Event order, sequence numbering and the event count never change.
 *
 * @param raw - the raw log bytes (plaintext JSONL or concatenated zstd frames).
 * @param compressed - whether the log is zstd-encoded.
 * @param knownTypes - the event types this harness understands; without a
 *   vocabulary (undefined or empty) the log is returned untouched.
 * @returns `{ bytes, events }` — the possibly identical log bytes, and every
 *   marked event in log order. `bytes === raw` means nothing had to change.
 */
export function markUnknownEventsIgnorable(raw, compressed, knownTypes) {
  if (!(knownTypes instanceof Set) || knownTypes.size === 0) return { bytes: raw, events: [] };
  const marked = [];
  if (!compressed) {
    const lines = raw.toString("utf8").split("\n");
    if (!markUnknownEventLines(lines, knownTypes, marked, 1)) return { bytes: raw, events: [] };
  ...
  for (let index = 0; index < frames.length; index++) {
    ...
    const bytes = raw.subarray(frame.start, frame.end);
    // The header frame is never rewritten: the backend requires it to be
    // exactly one header line, and re-compressing it would only risk that.
    if (index === 0) {
      out.push(bytes);
      continue;
    }
    ...
    try {
      ...
      // An undecodable frame means the rest of the file cannot be interpreted:
      // keep the remaining bytes verbatim instead of guessing at them.
      out.push(raw.subarray(frame.start));
      keptRest = true;
      break;
    }
    const lines = text.split("\n");
    if (!markUnknownEventLines(lines, knownTypes, marked, 0)) {
      out.push(bytes);
      continue;
    }
    changed = true;
  ...
  if (!changed) return { bytes: raw, events: [] };
  if (!keptRest && tornStart !== undefined) out.push(raw.subarray(tornStart));
  return { bytes: Buffer.concat(out), events: marked };
}

/**
 * Mark every unknown, unmarked event line in place.
 * @param lines - the decoded lines of one frame, or of a whole plaintext log.
 * @param knownTypes - the event types this harness understands.
 * @param marked - collects `{ type, seq }` for every marked event, in log order.
 * @param startIndex - 0 for an event-only frame, 1 for a whole log (header first).
 * @returns whether any line changed.
 */
function markUnknownEventLines(lines, knownTypes, marked, startIndex) {
  let changed = false;
  for (let i = startIndex; i < lines.length; i++) {
    ...
    try {
      ...
      continue; // never touch a line we cannot prove is an event
    }
    if (event === null || typeof event !== "object" || typeof event.type !== "string") continue;
    if (knownTypes.has(event.type) || event.ignorable === true) continue;
    // The writer's envelope order is `type, seq, time, data`; the marker goes last.
    lines[i] = JSON.stringify({ ...event, ignorable: true });
    marked.push({ type: event.type, seq: typeof event.seq === "number" ? event.seq : null });
    changed = true;
  }
  return changed;
}
...
/**
 * The event vocabulary of the harness this plugin runs inside, resolved through
 * the harness' own package so the plugin never has to carry a copy of a
 * vocabulary that grows with every DSH release. `undefined` when that package
 * is not resolvable from this plugin (an unusual install layout): the import
 * then behaves exactly as before and leaves foreign events untouched.
 */
async function loadKnownEventTypes() {
  try {
    ...
    return known instanceof Set && known.size > 0 ? known : undefined;
  } catch {
    return undefined;
  }
}
...
/**
 * Compact `{ type, count }` summary of the events an import had to mark. A
 * package normally carries both the legacy `session.jsonl` and the current
 * `session.vN.jsonl` generation, and both are marked, so the same event would
 * otherwise be counted twice; identity is `(type, seq)`. An event without a
 * numeric seq cannot be identified that way and is always counted.
 * Exported for tests.
 */
export function summarizeUnknownEvents(events) {
  const seen = new Set();
...
/**
 * The physical log encoding this machine's JSONL backend expects. Prefers the
 * backend's own setting; otherwise infers it from a session that already lives
 * here, so an import always matches the local root. `undefined` (an empty
 * machine, or a non-JSONL backend) leaves the package's own encoding in place.
 */
async function localLogEncoding(ctx, sessionsRoot, probe) {
  const persistence = ctx.get("sessionPersistence");
  const configured = persistence?.compression;
  if (configured === "zstd" || configured === "none") return configured;
  if (probe !== undefined) {
    try {
      const known = logEncodingOf(basename(persistence.locate(probe).path));
  ...
  try {
    for (const project of await readdir(sessionsRoot, { withFileTypes: true })) {
      ...
      for (const dir of await readdir(join(sessionsRoot, project.name), { withFileTypes: true })) {
        if (!dir.isDirectory()) continue;
        for (const file of await readdir(join(sessionsRoot, project.name, dir.name))) {
          const found = logEncodingOf(file);
          if (found !== undefined) return found;
        }
      }
    }
  } catch { /* an unreadable root leaves the package encoding untouched */ }
  return undefined;
}
...
async function backupSession(ctx, sessionId, targetDir, format) {
  ...
  // Collect the attachments the session log references and add them to the package.
  const attachmentMembers = [];
  const attachmentStore = attachmentStoreRoot();
  try {
    const attIds = collectAttachmentIds(await readFile(logPath));
    for (const id of attIds) {
      const hex = id.slice("sha256:".length);
      for (const rel of attachmentRelativePaths(hex)) {
        const abs = join(attachmentStore, ...rel.split("/"));
        if (await exists(abs)) {
          attachmentMembers.push({ name: `attachments/v1/${rel}`, data: await readFile(abs), rel });
          break;
        }
      }
    }
  } catch {
    // A log we cannot scan must not fail the whole export; the session files still ship.
  }
  const manifest = {
    schema: 1,
    ...
    logName,
    attachments: attachmentMembers.map((m) => m.rel),
  ...
  for (const m of attachmentMembers) members.push({ name: m.name, data: m.data });
 ...
 * machine resumable on another: the conversation's own events are untouched —
 * only the workspace anchor is replaced, plus the `ignorable` marker described
 * below for event types this build does not know.
 *
 * Steps:
 *   1. if `sourcePath` is a file, extract it (zip/tar.gz) to a temp dir;
 *   2. read manifest.json; require `sessionId` and the log file;
 *   3. rewrite the log header `cwd` to the canonical target path, and mark
 *      every event type this harness does not know as `ignorable` (a package
 *      from a newer harness would otherwise install a session this build then
 *      refuses to open, while the import reported success);
...
async function importSession(ctx, sourcePath, targetPath) {
  ...
  try {
    if (srcStat.isFile()) {
      ...
      await extractMembersToDir(members, tempDir);
      pkgDir = tempDir;
    }
    ...
    try {
      ...
      // No manifest.json — tolerate manually-zipped session folders by deriving
      // a manifest from the session log header (the import remaps cwd anyway).
      let logGuess;
      try {
        logGuess = (await readdir(pkgDir)).find((name) => /^session\.jsonl(\.zstd)?$/.test(name));
      } catch { logGuess = undefined; }
      if (!logGuess) throw fail("import-invalid-package", "Not a valid export package: missing or unreadable manifest.json");
      try {
        const header = await readLogHeader(join(pkgDir, logGuess));
        manifest = {
          sessionId: header.id,
          cwd: header.cwd ?? null,
          version: header.version,
    ...
    const declaredVersion = (await readLogHeader(sourceLog).catch(() => undefined))?.version ?? manifest.version;

    // `destDir` exists from here on, so every later failure has to remove it: a
    // half-installed session directory is exactly the stranded artifact the
    // checks below exist to prevent.
    const rollback = async (code, message) => {
      await rm(destDir, { recursive: true, force: true });
      throw fail(code, message);
    };

    // Install one log artifact: re-encode it to the local container, remap the
    // header `cwd`, and mark the event types this harness does not know (all
    // three steps are container-aware).
    const localEncoding = await localLogEncoding(ctx, sessionsRoot, probe);
    const knownTypes = await loadKnownEventTypes();
    const installedLogs = [];
    const foreignEvents = [];
    const installLog = async (entryName, bytes, isPrimary) => {
      const from = logEncodingOf(entryName) ?? (entryName.endsWith(".jsonl.zstd") ? "zstd" : "none");
      const to = localEncoding ?? from;
      // Everything that interprets the packaged bytes sits under one guard: a log
      // whose bytes are not the container its name promises (a hand-built package)
      // cannot be decoded at any of these steps, and the caller must get a coded
      // error rather than a raw codec message — and no stranded directory.
      let data;
      let outName;
      try {
        data = from === to ? bytes : convertLogEncoding(bytes, from, to);
        if (isPrimary) data = rewriteLogHeader(data, canonicalTarget, to === "zstd");
        outName = from === to ? entryName : reencodeLogName(entryName, to);
        // A package may name its log for a different generation than its header
        // declares (hand-built packages, older harnesses); install the primary
        // log under the name its header announces.
        if (isPrimary && Number.isSafeInteger(declaredVersion)) {
          const canonical = sessionLogName(declaredVersion, to);
          if (!(await exists(join(destDir, canonical)))) outName = canonical;
        }
        // A newer harness may have written event types this build does not know;
        // the backend refuses such a session unless they carry the marker.
        const compat = markUnknownEventsIgnorable(data, to === "zstd", knownTypes);
        if (compat.bytes !== data) {
          data = compat.bytes;
          foreignEvents.push(...compat.events);
        }
      } catch (error) {
        return rollback(
          "import-log-unreadable",
          `The packaged session log is not readable as a ${to === "zstd" ? "zstd" : "plaintext"} log: ${entryName} (${error instanceof Error ? error.message : String(error)})`
        );
      }
      await writeFile(join(destDir, outName), data);
      installedLogs.push(outName);
    };

    try {
      await installLog(logName, raw, true);

      for (const entry of await readdir(pkgDir, { withFileTypes: true })) {
        // `attachments/` belongs to the content-addressed store, not the session dir.
        if (entry.name === logName || entry.name === "manifest.json" || entry.name === "attachments") continue;
        if (entry.isFile() && logEncodingOf(entry.name) !== undefined) {
          // A sibling generation log (e.g. `session.v3.jsonl.zstd`) must match the
          // local container too, or the backend rejects this session directory.
          await installLog(entry.name, await readFile(join(pkgDir, entry.name)), false);
          continue;
        }
        await cp(join(pkgDir, entry.name), join(destDir, entry.name), { recursive: true, force: false });
      }

      // Never leave behind a session directory DSH's own reader would reject: a
      // foreign container or a filename/version disagreement makes the backend
      // fail for the WHOLE sessions root (every list call breaks), so roll the
      // install back instead of stranding it.
      for (const name of installedLogs) {
        const installed = await readLogHeader(join(destDir, name)).catch(() => undefined);
        if (installed === undefined) await rollback("import-log-unreadable", `The installed session log cannot be read back: ${name}`);
        const encoding = logEncodingOf(name);
        if (localEncoding !== undefined && encoding !== localEncoding) {
          await rollback("import-log-encoding", `The installed log does not use this machine's log encoding (${localEncoding}): ${name}`);
        }
        if (sessionLogName(installed.version, encoding) !== name) {
          await rollback("import-log-generation", `The installed log name does not match its header version: ${name}`);
        }
      }
    } catch (error) {
      // Any failure while installing (an unscannable artifact, a write error, a
      // rejected log) leaves no partial session directory behind.
      await rm(destDir, { recursive: true, force: true });
      throw error;
    }

    // Restore packaged attachments into <DSH_HOME>/attachments/v1 (content-addressed).
    let restoredAttachments = 0;
    const pkgAttachments = join(pkgDir, "attachments", "v1");
    if (await exists(pkgAttachments)) {
      try {
        restoredAttachments = await restoreAttachments(pkgAttachments, attachmentStoreRoot());
      } catch {
        // Attachment restore is best-effort: the session itself already landed.
      }
    }
    ...
    return {
      ...
      logs: installedLogs,
      logEncoding: localEncoding,
      unknownEvents: summarizeUnknownEvents(foreignEvents),
      workspaceId: targetEntity ? targetEntity.id : undefined,
      workspaceTitle: targetEntity ? targetEntity.title : undefined
    };
  } finally {
