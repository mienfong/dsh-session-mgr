...
async function importSession(ctx, sourcePath, targetPath) {
  ...
  try {
    ...
    const installLog = async (entryName, bytes, isPrimary) => {
      ...
      let data = from === to ? bytes : convertLogEncoding(bytes, from, to);
      if (isPrimary) data = rewriteLogHeader(data, canonicalTarget, to === "zstd");
      let outName = from === to ? entryName : reencodeLogName(entryName, to);
      // A package may name its log for a different generation than its header
      // declares (hand-built packages, older harnesses); install the primary log
      // under the name its header announces.
      ...
      // the backend refuses such a session unless they carry the marker. An
      // artifact whose bytes are not the container its name promises (a
      // hand-built package) cannot be scanned at all: report that as a coded
      // error instead of letting a raw Zstandard message escape.
      let compat;
      try {
        compat = markUnknownEventsIgnorable(data, to === "zstd", knownTypes);
      } catch (error) {
        return rollback(
