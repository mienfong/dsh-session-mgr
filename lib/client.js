...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS =
      ...
      ".dsm-format-row{display:flex;gap:8px;flex-wrap:wrap;}" +
      ".dsm-format-opt{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:4px 10px;cursor:pointer;font-size:12.5px;}" +
      ".dsm-format-opt:hover{border-color:var(--dsw-alias-brand-primary);}" +
    ...
    var STR = {
      en: {
        ...
        "moveModal.noOther": "No other workspace here — enter a folder path below, or add a workspace first.",
        ...
        "backupModal.formatLabel": "Archive format:",
        "backupModal.formatZip": "ZIP (.zip) — Windows",
        "backupModal.formatTargz": "tar.gz — Linux",
        "backupModal.hint": "Exports a PORTABLE archive file containing a manifest.json and the full session log (plus any artifacts). Copy it to another machine and use \"Import\" there; the import rewrites the cwd so the conversation resumes seamlessly.",
        ...
        "importModal.subtitle": "Import a portable session archive into this machine:",
        "importModal.sourceLabel": "Backup archive file (.zip or .tar.gz):",
        "importModal.targetLabel": "Target workspace or folder (the session's new cwd on this machine):",
        "importModal.hint": "The archive keeps the whole conversation; the import rewrites the cwd to the destination so the session resumes seamlessly here.",
        "importModal.cancel": "Cancel",
        "importModal.confirm": "Import",
        "importModal.busy": "Importing…",
        "note.imported": "Imported session {sid} to \"{path}\"",
        "note.importedCompat": " This package came from a newer DSH: {n} event(s) this version does not know ({types}) were marked skippable, so the conversation still loads here.",
        "page.import": "Import",
        ...
        "err.backup-format-invalid": "The archive format must be 'zip' or 'targz'",
        ...
        "err.import-archive-unsupported": "Unsupported archive format (expected .zip or .tar.gz)",
        "err.import-log-unreadable": "The imported session log is unreadable; the import was rolled back",
        "err.import-log-encoding": "The imported log does not use this machine's log encoding; the import was rolled back",
        "err.import-log-generation": "The imported log name does not match its header version; the import was rolled back",
      ...
      "zh-CN": {
        ...
        "moveModal.noOther": "此处没有其他工作区——请在下方输入文件夹路径，或先新建工作区。",
        ...
        "backupModal.formatLabel": "压缩格式：",
        "backupModal.formatZip": "ZIP (.zip) — Windows",
        "backupModal.formatTargz": "tar.gz — Linux",
        "backupModal.hint": "这会汇出一个**可携式压缩档**——内含 manifest.json 与完整会话日志（含附件）。复制到另一台电脑后用「汇入」，汇入会重设 cwd，会话即可无缝继续。",
        ...
        "importModal.subtitle": "把可携式会话压缩档汇入本机：",
        "importModal.sourceLabel": "备份压缩档文件（.zip 或 .tar.gz）：",
        "importModal.targetLabel": "目标工作区 / 文件夹（此机上该会话的新 cwd）：",
        "importModal.hint": "压缩档内含完整会话；汇入时会把 cwd 重设为目的地路径，会话即可在此机无缝继续。",
        "importModal.cancel": "取消",
        "importModal.confirm": "汇入",
        "importModal.busy": "汇入中…",
        "note.imported": "已汇入会话 {sid} 到「{path}」",
        "note.importedCompat": "。该包由更新版本的 DSH 写出：{n} 个本机不认识的事件（{types}）已标记为可跳过，对话仍可正常加载。",
        "page.import": "汇入",
        ...
        "err.backup-verify-failed": "备份失败：日志未写入目标文件",
        "err.backup-format-invalid": "压缩格式必须是 'zip' 或 'targz'",
        ...
        "err.import-archive-unsupported": "不支持的压缩格式（应为 .zip 或 .tar.gz）",
        "err.import-log-unreadable": "汇入的会话日志无法读取，已回滚本次汇入",
        "err.import-log-encoding": "汇入的日志与本机的日志编码不一致，已回滚本次汇入",
        "err.import-log-generation": "汇入的日志文件名与 header 版本不符，已回滚本次汇入",
        "header.btn": "移动到工作区",
      ...
      "zh-TW": {
        ...
        "moveModal.noOther": "此處沒有其他工作區——請在下方輸入資料夾路徑，或先新增工作區。",
        ...
        "backupModal.placeholder": "例如 D:\\backups\\dsh-sessions",
        "backupModal.formatLabel": "壓縮格式：",
        "backupModal.formatZip": "ZIP (.zip) — Windows",
        "backupModal.formatTargz": "tar.gz — Linux",
        "backupModal.hint": "這會匯出一個**可攜式壓縮檔**——內含 manifest.json 與完整會話日誌（含附件）。複製到另一台電腦後用「匯入」，匯入會重設 cwd，會話即可無縫繼續。",
        "backupModal.cancel": "取消",
        ...
        "importModal.subtitle": "把可攜式會話壓縮檔匯入本機：",
        "importModal.sourceLabel": "備份壓縮檔檔案（.zip 或 .tar.gz）：",
        "importModal.targetLabel": "目標工作區 / 資料夾（此機上該會話的新 cwd）：",
        "importModal.hint": "壓縮檔內含完整會話；匯入時會把 cwd 重設為目的地路徑，會話即可在此機無縫繼續。",
        "importModal.cancel": "取消",
        "importModal.confirm": "匯入",
        "importModal.busy": "匯入中…",
        "note.imported": "已匯入會話 {sid} 到「{path}」",
        "note.importedCompat": "。該包由更新版本的 DSH 寫出：{n} 個本機不認識的事件（{types}）已標記為可跳過，對話仍可正常載入。",
        "page.import": "匯入",
        ...
        "err.backup-verify-failed": "備份失敗：日誌未寫入目標檔案",
        "err.backup-format-invalid": "壓縮格式必須是 'zip' 或 'targz'",
        ...
        "err.import-archive-unsupported": "不支援的壓縮格式（應為 .zip 或 .tar.gz）",
        "err.import-log-unreadable": "匯入的會話日誌無法讀取，已回滾本次匯入",
        "err.import-log-encoding": "匯入的日誌與本機的日誌編碼不一致，已回滾本次匯入",
        "err.import-log-generation": "匯入的日誌檔名與 header 版本不符，已回滾本次匯入",
        "header.btn": "移動到工作區",
    ...
    /**
     * One extra sentence for an import that had to mark event types this build
     * does not know (a package written by a newer DSH), "" when nothing was
     * skipped.
     */
    function compatNote(res) {
      var unknown = res && Array.isArray(res.unknownEvents) ? res.unknownEvents : [];
      var total = 0;
      var types = [];
      for (var i = 0; i < unknown.length; i++) {
        total += Number(unknown[i].count) || 0;
        if (typeof unknown[i].type === "string" && types.indexOf(unknown[i].type) === -1) types.push(unknown[i].type);
      }
      if (total === 0) return "";
      return t("note.importedCompat", { n: String(total), types: types.join(", ") });
    }
      ...
      return react.createElement("div", { className: "dsm-overlay", onClick: function (e) { if (e.target === e.currentTarget && !props.busy) props.onClose(); } },
        react.createElement("div", { className: "dsm-modal" },
          ...
          (!props.workspaces || props.workspaces.length === 0)
            ? react.createElement("div", { className: "dsm-hint" }, t("moveModal.noOther"))
            : null,
    ...
    function BackupModal(props) {
      ...
      var _useState2 = react.useState(props.defaultFormat || "zip");
      var format = _useState2[0];
      var setFormat = _useState2[1];
      return react.createElement("div", { className: "dsm-overlay", onClick: function (e) { if (e.target === e.currentTarget && !props.busy) props.onClose(); } },
        react.createElement("div", { className: "dsm-modal" },
          react.createElement("div", { className: "dsm-modal-title" }, props.title),
          props.subtitle ? react.createElement("div", { className: "dsm-modal-sub" }, props.subtitle) : null,
          ...
          react.createElement("div", null, t("backupModal.formatLabel")),
          react.createElement("div", { className: "dsm-format-row" },
            [
              { id: "zip", label: t("backupModal.formatZip") },
              { id: "targz", label: t("backupModal.formatTargz") }
            ].map(function (opt) {
              return react.createElement("label", { key: opt.id, className: "dsm-format-opt" },
                react.createElement("input", { type: "radio", name: "dsm-backup-format", checked: format === opt.id, onChange: function () { setFormat(opt.id); } }),
          ...
          react.createElement("div", { className: "dsm-modal-actions" },
            ...
            react.createElement("button", {
              ...
              onClick: function () { props.onConfirm(dir.trim(), format); }
    ...
    function ImportModal(props) {
      var _useState = react.useState(props.defaultSourcePath || "");
      var sourcePath = _useState[0];
      var setSourcePath = _useState[1];
      var _useState2 = react.useState("");
      ...
      return react.createElement("div", { className: "dsm-overlay", onClick: function (e) { if (e.target === e.currentTarget && !props.busy) props.onClose(); } },
        react.createElement("div", { className: "dsm-modal" },
          ...
          react.createElement("input", {
            className: "dsm-input",
            placeholder: "D:\\backups\\session-xxxx.zip",
            value: sourcePath,
            onChange: function (e) { setSourcePath(e.target.value); }
          }),
          react.createElement("div", null, t("importModal.targetLabel")),
          react.createElement("input", {
            className: "dsm-input",
            placeholder: "D:\\_Program\\DeepSeek_Harness\\Chat",
            value: targetPath,
          ...
          react.createElement("div", { className: "dsm-modal-actions" },
            ...
            react.createElement("button", {
              className: "dsm-btn dsm-btn-primary",
              disabled: props.busy || sourcePath.trim().length === 0 || targetPath.trim().length === 0,
              onClick: function () { props.onConfirm(sourcePath.trim(), targetPath.trim()); }
    ...
    function MovePage() {
      ...
      function runBackup(ids, targetDir, format) {
        setMoving(true);
        setError(null);
        var cursor = Promise.resolve();
        var results = [];
        for (var i = 0; i < ids.length; i++) {
          (function (id) {
            cursor = cursor.then(function () {
              return api("backup", { sessionId: id, targetDir: targetDir, format: format })
                .then(function () { results.push({ id: id, ok: true }); })
                .catch(function (err) { results.push({ id: id, ok: false, error: describe(err) }); });
            });
          })(ids[i]);
      ...
      function runImport(sourcePath, targetPath) {
        setMoving(true);
        setError(null);
        api("import", { sourcePath: sourcePath, targetPath: targetPath })
          .then(function (res) {
            refreshNativeViews();
            setNote(t("note.imported", { sid: res.sessionId, path: res.cwd || targetPath }) + compatNote(res));
            setImportModal(null);
          })
      ...
      var modalEl = null;
      if (modal !== null) {
        // Destination options exclude any workspace the selected session(s) are
        // already in — moving to the current workspace is a no-op.
        var excludedWs = {};
        for (var mi = 0; mi < modal.ids.length; mi++) {
          var ms = sessions.find(function (x) { return x.id === modal.ids[mi]; });
          if (ms && ms.workspaceId) excludedWs[ms.workspaceId] = true;
        }
        var destWorkspaces = workspaces.filter(function (ws) { return !excludedWs[ws.id]; });
      ...
      if (backupModal !== null) {
        backupEl = react.createElement(BackupModal, {
          ...
          onConfirm: function (dir, format) { runBackup(backupModal.ids, dir, format); }
        });
      }
      var deleteEl = null;
      if (deleteModal !== null) {
      ...
      if (importModal !== null) {
        importEl = react.createElement(ImportModal, {
          ...
          onConfirm: function (sourcePath, targetPath) { runImport(sourcePath, targetPath); }
        });
      }
    ...
    function HeaderMoveAction(props) {
      ...
      function openModal() {
        ...
        setNote(null);
        api("list", {}).then(function (res) {
          var all = Array.isArray(res.workspaces) ? res.workspaces : [];
          var ses = Array.isArray(res.sessions) ? res.sessions : [];
          var current = null;
          for (var i = 0; i < ses.length; i++) {
            if (ses[i].id === sessionId && ses[i].workspaceId) { current = ses[i].workspaceId; break; }
          }
          // Exclude the session's current workspace (moving there is a no-op).
          setWorkspaces(current ? all.filter(function (ws) { return ws.id !== current; }) : all);
          setOpen(true);
        }).catch(function (err) {
          setError(describe(err));
        });
